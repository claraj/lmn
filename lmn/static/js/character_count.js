...
document.getElementById('counter').innerHTML = ((240 - show_info) + "/240");

// each time the title or text is typed in, the listener will activate, recounting the lengths of the two elements
// and including the original show_info length, and subtract from 240 (the tweet character limit)
document.getElementById('id_title').onkeyup = function () {
  document.getElementById('counter').innerHTML = (240 - (this.value.length + text_count.value.length + show_info)) + "/240"
};
document.getElementById('id_text').onkeyup = function () {
  document.getElementById('counter').innerHTML = (240 - (this.value.length + title_count.value.length + show_info)) + "/240"
};